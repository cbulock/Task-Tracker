...
$query = "
	SELECT c.name chore_name, date, c.id chore_id, u.name username
	FROM history h
...
<html>
	...
	<content>
		...
		<?
			foreach ($logs as $log) {
				echo "<tr><td>" . $log['date'] . "</td><td><a href='/chore?id=" . $log['chore_id'] . "'>" . $log['chore_name'] . "</a></td><td>" . $log['username'] . "</td></tr>";
			}
		?>
		</table>
	</content>
